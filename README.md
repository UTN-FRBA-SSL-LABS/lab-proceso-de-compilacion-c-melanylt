...
> **R:**Porque se incluye todo el código de los archivos indicados por las directivas #include y además, se reemplazan las macros.
...
> **R:**El comentario no aparece en el archivo .i, ya que este fue eliminado en el preprocesamiento
...
> **R:**(sin DEBUG) no devuelve nada | (con DEBUG) printf("[DEBUG] %s\n", ("Iniciando main"));
...
> **R:**Indican el número de línea y el archivo de origen del código. El bloque de printf proviene de stdio.h
...
> **R:**_sumar: pushl %ebp | movl %esp, %ebp | movl _llamadas, %eax | addl $1, %eax

Explicá en términos generales qué hacen esas instrucciones
(usá los comentarios del laboratorio como guía):

> **R:**pushl guarda un valor en la pila, movl copia un valor de un lugar a otro y addl suma valores
...
> **R:**_llamadas: | movl _llamadas, %eax | movl %eax, _llamadas | movl _llamadas, %eax
...
> **R:**En programa.o, usa tipo U porque necesita area_circulo pero no esta definido en el mismo. En cambio, en matematica.o tiene tipo T ya que es esta quien la define.
...
> **R:**Quedan porque son funciones de las bibliotecas dinámicas que no se incluyen dentro del ejecutable. Estas son resueltas por el cargador dinámico cuando el programa se inicia/ejecuta.
...
> **R:**sumar(3, 4)       = 7 | CUADRADO(5)      = 25 | MAX(7, 12)        = 12 | area_circulo(5.0) = 78.5398 | Factoriales:  0! = 1  1! = 1  2! = 2  3! = 6  4! = 24  5! = 120 | Llamadas a sumar(): 1
...
> **R:**Una macro función como CUADRADO (X) es un reemplazo de un nombre junto con sus argumentos por una expresión, la cual desaparece en la etapa de preprocesamiento. En cambio, una función real como sumar(a,b) es código que se compila y después se ejecuta, manteniendose todo el proceso hasta la etapa final. Además, a diferencia de las macros, las funciones reales tienen verificación de tipos.
...
> **R:**El símbolo tipo T es una función y está definida en la sección .text, mientras que, el tipo D es una variable global con valor inicial y está definida en la sección .data
...
> **R:**ntdll.dll => /c/WINDOWS/SYSTEM32/ntdll.dll (0x7ffec63e0000) | KERNEL32.DLL => /c/WINDOWS/System32/KERNEL32.DLL (0x7ffec4f40000) | KERNELBASE.dll => /c/WINDOWS/System32/KERNELBASE.dll (0x7ffec3a80000) | ucrtbase.dll => /c/WINDOWS/System32/ucrtbase.dll (0x7ffec37c0000)

¿Por qué `libc` no hubo que especificarla explícitamente al enlazar con `gcc`?

> **R:**Porque gcc ya la agrega automáticamente cuando hace el enlazado
